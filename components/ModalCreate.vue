...
export default Vue.extend({
  ...
  watch: {
    $route(route: Route) {
      if (!route.hash || route.hash !== '#modal-create') return
      this.toggle()
    },
  },
  methods: {
    ...
    doUpdate() {
      this.$store
        .dispatch('admin/createProduct', this.newProductData)
    ...
    <div class="flex items-center justify-center min-h-screen">
      ...
      >
        <div class="flex w-full justify-center justify-between">
          <h3 class="font-bold text-2xl">Criar um Produto</h3>
          <button @click="toggle">
            <svg
        ...
        <div class="py-4">
          <form @submit.prevent="() => {}">
            <GenericInput
              ...
              label="Nome"
            />
            <div class="flex flex items-center justify-between gap-4">
              <GenericInput
                id="price"
